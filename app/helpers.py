...
def set_response_headers(app: web.Application):
    ...
    csp = {
        ...
        "script-src": [
            ...
            "'sha256-bfloDFhW9eAYHv7CGM+kIiD7H2F+b/hGF5Wj8LOnLyo='",  # plausible("404",{props:{path:document.location.pathname}})
            "'sha256-MUo3BR9SqVxUnxV7Dw9uvwDu81yLUU2qKuLSqkGuXmE='",  # plausible("500",{props:{path:document.location.pathname}})
            "'sha256-twm5+6fB+DAXzyHpRxOLTU4v98ckfIQjB7Zcgw3XEHs='",  # plausible("Login failed",{props:{plan:"{{ plan_id }}"}})
        ],
