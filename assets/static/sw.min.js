...
self.addEventListener("error",a=>{const b={type:Object.getPrototypeOf(a.error).name};Object.getOwnPropertyNames(a.error).forEach(d=>b[d]=a.error[d]);fetch("/api/report-error",{method:"post",body:new URLSearchParams({message:a.message,filename:a.filename,lineno:a.lineno,colno:a.colno,error:JSON.stringify(b)})})});
