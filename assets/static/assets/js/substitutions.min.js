...
if(window.location.hash.startsWith("#timetable:"))try{let [,a,b]=window.location.hash.split(":");b=atob(b);let c=!0,f;if(150!==b.length)console.warn("Timetable in URL has wrong length:",b.length,"instead of",150,b),c=!1;else{f=[];for(let d=0;5>d;d++){let n=[];f.push(n);for(let h=0;10>h;h++){let t=b.substr(30*d+3*h,3).trim();n.push(t)}}}if(c){let d;try{(d=JSON.parse(window.localStorage.getItem(g+"-timetables")))||(d={})}catch{d={}}a=a.toUpperCase();let n=a in d?"Die aufgerufene URL enth\u00e4lt einen Stundenplan f\u00fcr "+
a+". Soll der aktuell gespeicherte Stundenplan f\u00fcr "+a+" durch diesen ersetzt werden?":"Die aufgerufene URL enth\u00e4lt einen Stundenplan f\u00fcr "+a+". Diesen Stundenplan setzen?";if(k){let h=!1;for(let t of k.split(", "))if(t.toUpperCase()===a){h=!0;break}h||(n+=" Achtung: Der Stundenplan wird erst angewendet, wenn "+a+" auch ausgew\u00e4hlt ist.")}else n+=" Achtung: Der Stundenplan wird erst angewendet, wenn Vertretungen ausgew\u00e4hlt sind.";confirm(n)&&(d[a]=f,window.localStorage.setItem(g+
"-timetables",JSON.stringify(d)));window.location.hash="";plausible("Timetable: Set From Link")}}catch(a){console.error("Error while retrieving timetable from URL",a),e(a)};const l=document.getElementsByClassName("date");
...
break}}}m();window.addEventListener("focus",()=>m());let p,q=document.getElementById("status").textContent;try{if(p=JSON.parse(window.localStorage.getItem(g+"-seen-substitutions")),p.status!==q){let a=Date.now();for(let b of Object.keys(p.seenSubstitutions))b<=a&&delete p.seenSubstitutions[b];for(let [b,c]of Object.entries(p.newSubstitutions))b>a&&(b in p.seenSubstitutions?p.seenSubstitutions[b].push(...c):p.seenSubstitutions[b]=c);p.newSubstitutions={};p.status=q}}catch{}p||(p={seenSubstitutions:{},newSubstitutions:{},status:q});
for(let a of document.getElementsByClassName("substitutions-box")){const b=a.querySelector(".substitutions-table tbody");if(b){const c=a.querySelector(".date").textContent.trim();let [,f,d,n]=c.match(/(\d\d?).(\d\d?).(\d\d\d\d)/);const h=Date.UTC(n,d-1,f+1);h in p.seenSubstitutions||(p.seenSubstitutions[h]=[]);h in p.newSubstitutions||(p.newSubstitutions[h]=[]);let t;for(let A of b.children){let D=A.querySelector(".group-name");null!=D&&(t=D.textContent.trim());let w=t;for(let E of A.children)E.classList.contains(".group-name")||
(w+="#"+E.textContent.trim());p.seenSubstitutions[h].includes(w)||(A.classList.add("new-subs"),p.newSubstitutions[h].includes(w)||p.newSubstitutions[h].push(w))}}}window.localStorage.setItem(g+"-seen-substitutions",JSON.stringify(p));const r=document.getElementById("toggle-notifications"),u=document.getElementById("notifications-info"),v=document.getElementById("notifications-info-none"),x=document.getElementById("notifications-info-all"),y=document.getElementById("notifications-info-selection"),z=document.getElementById("notifications-info-blocked"),B=document.getElementById("notifications-info-failed");
...
function H(a,b){console.log("Setting notification-state to",a);G=a;"failed"!==a&&localStorage.setItem(g+"-notification-state",G);localStorage.setItem(g+"-notification-state-all",G);switch(G){case "granted-and-enabled":r.checked=!0;r.disabled=!1;u.innerHTML=""!==k?y.innerHTML.replace("{selection}",k):x.innerHTML;F(!0,b).catch(c=>{console.error("Push subscription failed",c);H("failed",b)});break;case "denied":r.checked=!1;r.disabled=!0;u.innerHTML=z.innerHTML;break;case "failed":r.checked=!1;r.disabled=
!0;u.innerHTML=B.innerHTML;break;case "granted-and-disabled":F(!1,b).catch(c=>{console.error("Push subscription failed",c);H("failed",b)});r.checked=!1;r.disabled=!1;u.innerHTML=v.innerHTML;break;default:case "default":r.checked=!1,r.disabled=!1,u.innerHTML=v.innerHTML}}
...
"serviceWorker"in navigator?window.addEventListener("load",()=>{navigator.serviceWorker.ready.then(a=>{console.log("ServiceWorker is active:",a.active);"Notification"in window&&"localStorage"in window&&"PushManager"in window?I(a):(console.warn("Push Notifications are not supported"),localStorage.setItem(g+"-notification-state-all","unsupported"))});navigator.serviceWorker.register("/sw.min.js").then(a=>{console.log("ServiceWorker registration successful:",a)}).catch(a=>console.warn("ServiceWorker registration failed:",
a))}):(console.warn("Service Worker is not supported"),localStorage.setItem(g+"-notification-state-all","unsupported"));const J=document.getElementById("online-status");let K=null;function L(){J.textContent="Aktuell";J.classList.add("online");J.classList.remove("offline","updating")}function M(){J.textContent="Offline";J.classList.add("offline");J.classList.remove("online","updating")}
