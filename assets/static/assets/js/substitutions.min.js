...
window.addEventListener("error",a=>{e(a.error,a)});window.addEventListener("unhandledrejection",a=>{console.log("unhandledrejection",a);e(a.reason)});const k=window.location.pathname.split("/",2)[1],l=document.getElementById("selectionInput").value;
if(window.location.hash.startsWith("#timetable:"))try{let [,a,b]=window.location.hash.split(":");b=atob(b);let c=!0,f;if(150!==b.length)console.warn("Timetable in URL has wrong length:",b.length,"instead of",150,b),c=!1;else{f=[];for(let d=0;5>d;d++){let h=[];f.push(h);for(let g=0;10>g;g++){let n=b.substr(30*d+3*g,3).trim();""===n&&(n=null);h.push(n)}}}if(c){let d;try{(d=JSON.parse(window.localStorage.getItem(k+"-timetables")))||(d={})}catch{d={}}a=a.toUpperCase();let h=a in d?"Die aufgerufenen URL enth\u00e4lt einen Stundenplan f\u00fcr "+
a+". Soll der aktuell gesetzte Stundenplan f\u00fcr "+a+" durch diesen ersetzt werden?":"Die aufgerufene URL enth\u00e4lt einen Stundenplan f\u00fcr "+a+". Diesen Stundenplan setzen?";if(l){let g=!1;for(let n of l.split(", "))if(n.toUpperCase()===a){g=!0;break}g||(h+=" Achtung: Der Stundenplan wird erst angewendet, wenn "+a+" auch ausgew\u00e4hlt ist.")}else h+=" Achtung: Der Stundenplan wird erst angewendet, wenn Vertretungen ausgew\u00e4hlt sind.";confirm(h)&&(d[a]=f,window.localStorage.setItem(k+
"-timetables",JSON.stringify(d)));window.location.hash=""}}catch(a){console.error("Error while retrieving timetable from URL",a),e(a)};const m=document.getElementsByClassName("date");
...
for(let a of document.getElementsByClassName("substitutions-box")){const b=a.querySelector(".substitutions-table tbody");if(b){const c=a.querySelector(".date").textContent.trim();let [,f,d,h]=c.match(/(\d\d?).(\d\d?).(\d\d\d\d)/);const g=Date.UTC(h,d-1,f+1);g in q.seenSubstitutions||(q.seenSubstitutions[g]=[]);g in q.newSubstitutions||(q.newSubstitutions[g]=[]);let n;for(let z of b.children){let B=z.querySelector(".group-name");null!=B&&(n=B.textContent.trim());let v=n;for(let C of z.children)C.classList.contains(".group-name")||
(v+="#"+C.textContent.trim());q.seenSubstitutions[g].includes(v)||(z.classList.add("new-subs"),q.newSubstitutions[g].includes(v)||q.newSubstitutions[g].push(v))}}}window.localStorage.setItem(k+"-seen-substitutions",JSON.stringify(q));const t=document.getElementById("toggle-notifications"),u=document.getElementById("notifications-info"),w=document.getElementById("notifications-info-none"),x=document.getElementById("notifications-info-all"),y=document.getElementById("notifications-info-selection"),A=document.getElementById("notifications-info-blocked"),D=document.getElementById("notifications-info-failed");
...
function F(a,b){return new Promise((c,f)=>{b.pushManager.subscribe({userVisibleOnly:!0,applicationServerKey:E("BDu6tTwQHFlGb36-pLCzwMdgumSlyj_vqMR3I1KahllZd3v2se-LM25vhP3Yv_y0qXYx_KPOVOD2EYTaJaibzo8")}).then(d=>{console.log("Got push subscription:",d,a?"(active)":"(not active)");fetch(window.location.origin+window.location.pathname+"api/subscribe-push",{method:"post",headers:{"Content-Type":"application/json"},body:JSON.stringify({subscription:d.toJSON(),selection:l,is_active:a})}).then(h=>h.json()).then(h=>
{h.ok?(console.log("Push subscription successful"),c()):(console.error("Push subscription failed",h),f())})}).catch(d=>{console.error("Push subscription failed",d);f(d)})})}let G;
function H(a,b){console.log("Setting notification-state to",a);G=a;window.localStorage.setItem(k+"-notification-state",G);switch(G){case "granted-and-enabled":t.checked=!0;t.disabled=!1;u.innerHTML=""!==l?y.innerHTML.replace("{selection}",l):x.innerHTML;F(!0,b).catch(c=>{console.error("Push subscription failed",c);H("failed",b)});break;case "denied":t.checked=!1;t.disabled=!0;u.innerHTML=A.innerHTML;break;case "failed":t.checked=!1;t.disabled=!0;u.innerHTML=D.innerHTML;break;case "granted-and-disabled":F(!1,
...
G&&H("granted-and-disabled",a)});window.addEventListener("focus",b);G=window.localStorage.getItem(k+"-notification-state");null==G&&(G="default");b()||H(G,a)}
