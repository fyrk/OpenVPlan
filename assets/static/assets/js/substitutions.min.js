...
"Freitag"][b];c.setAttribute("type","text");c.setAttribute("autocapitalize","characters");c.setAttribute("autocomplete","off");c.setAttribute("autocorrect","off");c.setAttribute("spellcheck","false");c.setAttribute("maxlength","3");c.setAttribute("aria-label",`${k}. Stunde ${t}`);c.addEventListener("focus",u=>u.target.select());c.addEventListener("input",u=>{Q(u.target,u.target.dataset.oldvalue,!1);Q(u.target);u.target.dataset.oldvalue=u.target.value;a[u.target.dataset.selection][parseInt(u.target.dataset.weekday)][parseInt(u.target.dataset.lesson)-
1]=u.target.value.toUpperCase();window.localStorage.setItem(r+"-timetables",JSON.stringify(a))});c.dataset.selection=m;c.dataset.weekday=b;c.dataset.weekdayName=t;c.dataset.lesson=k;let E=a[m][b][k-1];null!=E?(c.value=E,c.dataset.oldvalue=E,Q(c)):c.dataset.oldvalue=""}h.addEventListener("keydown",b=>{switch(b.key){case "Tab":T(b.target,b.shiftKey?-1:1)&&b.preventDefault();break;case "ArrowDown":T(b.target,1)&&b.preventDefault();break;case "ArrowUp":T(b.target,-1)&&b.preventDefault();break;case "ArrowLeft":0!==
b.target.selectionStart||1!==b.target.selectionEnd&&0!==b.target.selectionEnd||S(b.target,-1)&&b.preventDefault();break;case "ArrowRight":b.target.selectionStart===b.target.selectionEnd&&S(b.target,1)&&b.preventDefault()}});h.addEventListener("input",b=>{b.target.value.length>=b.target.maxLength&&T(b.target,1)})}}}}catch(a){console.error(a)};
