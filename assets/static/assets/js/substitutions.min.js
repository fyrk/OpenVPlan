...
return!0};const q=document.createElement("tr");v.appendChild(q);const f=document.createElement("th");q.appendChild(f);f.innerText=g+".";for(let b=0;5>b;b++){const h=document.createElement("td");q.appendChild(h);const c=document.createElement("input");h.appendChild(c);c.classList.add("form-control","form-control-sm");c.id=n+"-"+b+"-"+g;const k=["Montag","Dienstag","Mittwoch","Donnerstag","Freitag"][b];c.setAttribute("type","text");c.setAttribute("maxlength","3");c.setAttribute("aria-label",`${g}. Stunde ${k}`);
c.addEventListener("focus",u=>u.target.select());c.addEventListener("input",u=>{P(u.target,u.target.dataset.oldvalue,!1);P(u.target);u.target.dataset.oldvalue=u.target.value;a[u.target.dataset.selection][parseInt(u.target.dataset.weekday)][parseInt(u.target.dataset.lesson)-1]=u.target.value.toUpperCase();window.localStorage.setItem(t+"-timetables",JSON.stringify(a))});c.dataset.selection=n;c.dataset.weekday=b;c.dataset.weekdayName=k;c.dataset.lesson=g;let D=a[n][b][g-1];null!=D?(c.value=D,c.dataset.oldvalue=
D,P(c)):c.dataset.oldvalue=""}v.addEventListener("keydown",b=>{switch(b.key){case "Tab":S(b.target,b.shiftKey?-1:1)&&b.preventDefault();break;case "ArrowDown":S(b.target,1)&&b.preventDefault();break;case "ArrowUp":S(b.target,-1)&&b.preventDefault();break;case "ArrowLeft":0!==b.target.selectionStart||1!==b.target.selectionEnd&&0!==b.target.selectionEnd||R(b.target,-1)&&b.preventDefault();break;case "ArrowRight":b.target.selectionStart===b.target.selectionEnd&&R(b.target,1)&&b.preventDefault()}});v.addEventListener("input",
b=>{b.target.value.length>=b.target.maxLength&&S(b.target,1)})}}}}catch(a){console.error(a)};
