...
for(let a of document.getElementsByClassName("substitutions-box")){const b=a.querySelector(".substitutions-table tbody");if(b){const c=a.querySelector(".date").textContent.trim();let [,d,g,f]=c.match(/(\d\d?).(\d\d?).(\d\d\d\d)/);const k=Date.UTC(f,g-1,d+1);k in r.seenSubstitutions||(r.seenSubstitutions[k]=[]);k in r.newSubstitutions||(r.newSubstitutions[k]=[]);let n;for(let B of b.children){let E=B.querySelector(".group-name");null!=E&&(n=E.textContent.trim());let x=n;for(let F of B.children)F.classList.contains(".group-name")||
(x+="#"+F.textContent.trim());r.seenSubstitutions[k].includes(x)||(B.classList.add("new-subs"),r.newSubstitutions[k].includes(x)||r.newSubstitutions[k].push(x))}}}window.localStorage.setItem(h+"-seen-substitutions",JSON.stringify(r));const u=document.getElementById("toggle-notifications"),v=document.getElementById("notifications-info"),w=document.getElementById("notifications-info-none"),y=document.getElementById("notifications-info-all"),z=document.getElementById("notifications-info-selection"),A=document.getElementById("notifications-info-blocked"),C=document.getElementById("notifications-info-failed");
...
"serviceWorker"in navigator?window.addEventListener("load",()=>{navigator.serviceWorker.ready.then(a=>{console.log("ServiceWorker is active:",a.active);"Notification"in window&&"localStorage"in window&&"PushManager"in window?J(a):(console.warn("Push Notifications are not supported"),m("notifications","unsupported"))});navigator.serviceWorker.register("/sw.min.js").then(a=>{console.log("ServiceWorker registration successful:",a)}).catch(a=>console.warn("ServiceWorker registration failed:",a))}):(console.warn("serviceWorker is not supported"),
m("notifications","unsupported"));const K=document.getElementById("online-status");let L=null;function M(){K.textContent="Aktuell";K.classList.add("online");K.classList.remove("offline","updating")}function N(){K.textContent="Offline";K.classList.add("offline");K.classList.remove("online","updating")}
...
window.addEventListener("online",()=>{console.log("online, checking for new substitutions");P()});window.addEventListener("offline",()=>{console.log("offline");N()});document.getElementById("themes-block").appendChild(document.getElementById("themes-block-template").content);const Q=document.documentElement,R=document.getElementById("themes-system-default"),S=document.getElementById("themes-light"),T=document.getElementById("themes-dark");function U(a){"system-default"!==a&&a||(a="system-"+(window.matchMedia&&window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"));m("theme",a)}
function V(a){localStorage.setItem("theme",a);switch(a){case "system-default":Q.classList.remove("light","dark");break;case "light":Q.classList.add("light");Q.classList.remove("dark");break;case "dark":Q.classList.add("dark"),Q.classList.remove("light")}U(a)}let W=localStorage.getItem("theme");switch(W){case "light":S.checked=!0;break;case "dark":T.checked=!0}R.addEventListener("change",()=>V("system-default"));S.addEventListener("change",()=>V("light"));T.addEventListener("change",()=>V("dark"));
U(W);}).call(this);
