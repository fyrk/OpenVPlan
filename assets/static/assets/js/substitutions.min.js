...
try{let a;try{(a=JSON.parse(window.localStorage.getItem(t+"-timetables")))||(a={})}catch(e){a={}}const d=document.getElementById("timetables-block");if(null!=d&&H){var P=function(n,p=null,u=!0){p||(p=n.value);var g=n.dataset.selection;if(g in e){g=e[g];let q=n.dataset.weekdayName;if(q in g&&(p=p.toUpperCase(),g=g[q][n.dataset.lesson-1],p in g))for(let f of g[p])u?f.classList.add("is-relevant"):f.classList.remove("is-relevant")}};const e={};for(let n of document.getElementsByClassName("substitutions-box")){const p=
n.querySelector(".substitutions-table tbody");if(p){var Q=function(){var f=[];var b=g.match(/^(\d+)([A-Za-z]*)$/);if(null!=b){var h=b[1];b=b[2]}else h="",b=g;if(""!==h)if(""!==b)for(let c of b)f.push(h+c);else f.push(h);else f.push(b);h=[];for(let c of H.split(", "))for(let k of f)if(k.includes(c)){h.push(c);break}console.log(g,f,h);for(let c of h)if(c in e||(e[c]={}),u in e[c])for(f=0;10>f;f++)e[c][u][f]=Object.assign(e[c][u][f],q[f]);else e[c][u]=q};const u=n.querySelector(".day-name").textContent;
...
p.querySelector(".timetable-selection").innerText=n;const u=p.querySelector("tbody");for(let g=1;11>g;g++){var R=function(b,h){let c=parseInt(b.dataset.weekday),k=parseInt(b.dataset.lesson);c+=h;return 0<=c&&4>=c?(document.getElementById(b.dataset.selection+"-"+c+"-"+k).focus(),!0):!1},S=function(b,h){let c=parseInt(b.dataset.weekday),k=parseInt(b.dataset.lesson);k+=h;if(1>k){c--;if(0>c)return!1;k=10}else if(10<k){c++;if(5<=c)return!1;k=1}document.getElementById(b.dataset.selection+"-"+c+"-"+k).focus();
return!0};const q=document.createElement("tr");u.appendChild(q);const f=document.createElement("th");q.appendChild(f);f.innerText=g+".";for(let b=0;5>b;b++){const h=document.createElement("td");q.appendChild(h);const c=document.createElement("input");h.appendChild(c);c.classList.add("form-control","form-control-sm");c.id=n+"-"+b+"-"+g;const k=["Montag","Dienstag","Mittwoch","Donnerstag","Freitag"][b];c.setAttribute("type","text");c.setAttribute("maxlength","3");c.setAttribute("aria-label",`${g}. Stunde ${k}`);
c.addEventListener("focus",v=>v.target.select());c.addEventListener("input",v=>{P(v.target,v.target.dataset.oldvalue,!1);P(v.target);a[v.target.dataset.selection][parseInt(v.target.dataset.weekday)][parseInt(v.target.dataset.lesson)-1]=v.target.value.toUpperCase();window.localStorage.setItem(t+"-timetables",JSON.stringify(a))});c.dataset.selection=n;c.dataset.weekday=b;c.dataset.weekdayName=k;c.dataset.lesson=g;let D=a[n][b][g-1];null!=D?(c.value=D,c.dataset.oldvalue=D,P(c)):c.dataset.oldvalue=""}u.addEventListener("keydown",
b=>{switch(b.key){case "Tab":S(b.target,b.shiftKey?-1:1)&&b.preventDefault();break;case "ArrowDown":S(b.target,1)&&b.preventDefault();break;case "ArrowUp":S(b.target,-1)&&b.preventDefault();break;case "ArrowLeft":0!==b.target.selectionStart||1!==b.target.selectionEnd&&0!==b.target.selectionEnd||R(b.target,-1)&&b.preventDefault();break;case "ArrowRight":b.target.selectionStart===b.target.selectionEnd&&R(b.target,1)&&b.preventDefault()}});u.addEventListener("input",b=>{b.target.value.length>=b.target.maxLength&&
S(b.target,1)})}}}}catch(a){console.error(a)};
