...
{const d=m.querySelector(".share-timetable-block");if(d.hidden){const l=d.querySelector(".timetable-link-input"),c=d.querySelector(".copy-timetable-link");l.value=(new URL("/"+u+"/#timetable:"+h+":"+btoa(JSON.stringify(v[h])),window.location.origin)).href;l.addEventListener("click",e=>e.target.select());let f=null;c.addEventListener("click",()=>{function e(){null!=f&&clearTimeout(f);f=setTimeout(()=>{c.classList.remove("copied");c.title="Kopieren";f=null},2E3)}navigator.clipboard.writeText(l.value).then(()=>
{c.classList.add("copying-failed");c.title="Kopieren fehlgeschlagen";e()}).catch(()=>{c.classList.add("copied");c.title="Kopiert!";e()})});d.hidden=!1}else d.hidden=!0});const g=m.querySelector("tbody");for(let d=1;11>d;d++){const l=document.createElement("tr");g.appendChild(l);const c=document.createElement("th");l.appendChild(c);c.innerText=d+".";for(let a=0;5>a;a++){const k=document.createElement("td");l.appendChild(k);const b=document.createElement("input");k.appendChild(b);b.classList.add("form-control",
...
p=>{w(p.target,p.target.dataset.oldvalue,!1);w(p.target);p.target.dataset.oldvalue=p.target.value;v[p.target.dataset.selection][parseInt(p.target.dataset.weekday)][parseInt(p.target.dataset.lesson)-1]=p.target.value.toUpperCase();window.localStorage.setItem(u+"-timetables",JSON.stringify(v))});b.dataset.selection=h;b.dataset.weekday=a;b.dataset.weekdayName=n;b.dataset.lesson=d;let x=v[h][a][d-1];null!=x?(b.value=x,b.dataset.oldvalue=x,w(b)):b.dataset.oldvalue=""}function f(a,k){let b=parseInt(a.dataset.weekday),
...
break;case "ArrowDown":f(a.target,1)&&a.preventDefault();break;case "ArrowUp":f(a.target,-1)&&a.preventDefault();break;case "ArrowLeft":0!==a.target.selectionStart||1!==a.target.selectionEnd&&0!==a.target.selectionEnd||e(a.target,-1)&&a.preventDefault();break;case "ArrowRight":a.target.selectionStart===a.target.selectionEnd&&e(a.target,1)&&a.preventDefault()}});g.addEventListener("input",a=>{a.target.value.length>=a.target.maxLength&&f(a.target,1)})}}};}).call(this);
