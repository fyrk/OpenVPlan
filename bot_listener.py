...
class BotUsers:
    ...
    def save(self):
        with open("data/bot_users.json", "w") as f:
        ...
        if selected_classes:
            ...
            if len(selected_classes) == 1:
                bot.send_message(message.chat.id, 'Die von dir auf der Webseite ausgewählte Klasse '
                                                  ...
                                                  'die Auswahl mit dem /klassen-Befehl anpassen. '.format(selected_classes[0]), parse_mode="html")
            else:
                bot.send_message(message.chat.id, 'Die von dir auf der Webseite ausgewählte Klassen '
                                                  '<i>{}</i> wurden automatisch ausgewählt. Du '
                                                  'kannst die Auswahl mit dem /klassen-Befehl anpassen. '.format(", ".join(selected_classes)),
                                 parse_mode="html")
    except Exception:
        pass
...
def all_messages(message):
    ...
    if status == "select-classes":
        ...
        if len(selected_classes) == 0:
            bot_users.set(message.chat.id, "status", "select-classes")
            bot.send_message(message.chat.id, "Bitte gib alle Klassen, für die du benachrichtigt werden willst, durch Kommata getrennt ein. ")
        elif len(selected_classes) == 1:
            bot.send_message(message.chat.id, 'Du wirst für die Klasse <i>{}</i> benachrichtigt. '.format(selected_classes[0]),
                             parse_mode="html")
        else:
            bot.send_message(message.chat.id, 'Du wirst für die Klassen <i>{}</i> benachrichtigt. '.format(", ".join(selected_classes)), parse_mode="html")
    else:
